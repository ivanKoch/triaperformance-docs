...
>
> ✅ ***Re-confirmed by Iván on September 5, 2026***, when the full register sweep reached this doc and put the exemption back to him explicitly rather than applying it. **`sales-playbook.md` was not converted.** *One consequence to know about, so it does not read as drift: the forwardable blurb in §B9b says `Acá te puedes poner en contacto`, and the same blurb rendered on `/referidos/` was changed to `Aquí` — a published page follows `brand-guidelines.md` §8, a WhatsApp message does not.* **The four `acá` in this file are deliberate.**
