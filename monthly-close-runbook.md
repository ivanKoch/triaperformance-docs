...
| `software` | Anthropic/Claude, **Google AI Studio / Gemini API**, TrainingPeaks coach account, Loom, design tools, anything on a card | **n8n, Twenty, Caddy, Postgres are self-hosted and cost $0** — list them at zero anyway, so the saving is visible and so a future move to a paid tier shows up as a change rather than a surprise. ***Gemini added Aug 30, 2026** — it was the one recurring charge on this list that no category named, while being the only one that varies with usage rather than sitting fixed. **Auto-reload has been on since Aug 10 ($7 floor, $30 top-up), so it no longer fails closed on a spend spike** — which is precisely why it needs a monthly line rather than a glance at a chart.* |
...
| GA4: sessions, top landing pages, conversion events | GA4 / BigQuery export | Yes — export already live, and **as of Aug 29, 2026 it also reaches the VPS**: `automation/analytics/sync_ga4_data.py` lands page-, event- and country-grain rows in `analytics-postgres`. *"Export already live" was true and still left the data unreachable from the box — a live export nothing reads is not an automatable row.* **✅ Verified live August 30, 2026** (`2026-07-21` onward — the property's whole life). 🚨 ***Read the `_clean` views, never the raw tables*** — `ga4_page_clean`, `ga4_event_clean`, `ga4_traffic_clean`, and `ga4_channel_day`, which filters already. **At least 25% of sessions in the only GA4 data that exists were Iván's own, 46% once two devices were confirmed by hand.** *And state the residual: **`ga4_selftraffic_month`** *(added Aug 31, 2026)* reports `external_sessions` alongside `internal_pct`, which is the confidence attached to it — the close quotes both, because the exclusion is only as good as four narrow rules over a cookie.* ⚠️ **A falling `internal_pct` is not good news**: it more likely means the rules stopped catching him than that he stopped visiting. *The GA4-side internal-traffic rule has never once tagged an event — `open-loops.md` NOW.* 🚨 **Measured Aug 31, 2026: July 59.3% internal, August 40.2%.** *Treat `internal_pct` as a* ***floor***, *not a measurement — it is what four cookie-bound rules caught.* 🚨 ***And check `partial_month` before computing any rate: the property was created 2026-07-21, so July is ELEVEN DAYS.*** *External sessions read 33 → 110 as months and 3.0 → 3.5 per day. **Both correct, one true** — §1(b) arriving through a different door. The view now carries `days_covered`, `partial_month` and `external_per_day`.* |
| GBP: views, searches, actions, new reviews | ~~Google Business Profile~~ **`analytics-postgres`** | ~~Yes — Performance API *(API approved Aug 17, 2026; script not written — until then this row is a manual screenshot, and a screenshot cannot be re-derived next month)*~~ **✅ Yes, and LIVE — `sync_gbp_data.py` on a 06:15 cron since Aug 30, 2026, first unattended run confirmed Aug 31.** *This row is no longer a screenshot. **Reviews are the row that earns its place** — 9 in July, 1 in March, zero every other month, 46 total, all replied — which turns §4's «review velocity, not review count» from an argument into a pulled series. **GBP is a credibility surface, not a channel**: 60–96 views/month, 2–3 website clicks, zero calls or direction requests in six months; report the counts and do not build a story on the trend.* ⚠️ **Two timing rules for the keyword row, both verified Aug 31, 2026.** *(1)* ***Do not quote an August keyword figure from an August 31 pull***: **GBP publishes a month's keywords after the month closes**, so 2026-08 read empty on the 31st. It self-heals — the sync's window reaches the first of the previous month, so any nightly run from **~Sept 8** collects it. *(2)* *Six months are genuinely empty at Google — 2025-02, 2025-12, 2026-02, 2026-03, 2026-06 — re-fetched and confirmed, not an artefact.* **Report them as empty and do not chase them.** *Most present months are heavily thresholded (Google returning «fewer than N» rather than a count), so `sum(value)` is NULL for many; count keywords, not impressions.* |
| GSC: impressions, clicks, ~~average position~~ **near-miss query count (positions 5–20)**, coverage errors | Search Console | ~~**Not today**~~ **Script written Aug 29, 2026** — `automation/analytics/sync_gsc_data.py`, Search Analytics API, nightly into `analytics-postgres`, with a 16-month backfill. *The original note was right about the cause: the GSC↔GA4 link is reporting-level only and a real pull does need the API. That API is now built.* ~~Pending Iván's setup + one verification query.~~ **✅ LIVE and verified August 30, 2026** — history back to `2025-05-06`. *This row is no longer a screenshot.* Owner: `ai-infrastructure-documentation.md` §9 |

> 🚨 ***Average position came off this scoreboard September 2, 2026, and the reason generalises.*** *August read* **10.01 → 38.80** *while impressions went up* **8×**. *Both numbers are correct and reading them together as "we got much worse" is wrong: average position is impression-weighted, so every new query the site starts appearing for — necessarily deep in the results at first — drags the average down* ***while being unambiguously good news***. **The metric moves against you exactly when the thing you want is happening.** *Replaced by the count of queries in the near-miss band (positions 5–20), which is already the view `gsc_near_miss` and is the actual "are we close to breaking through" signal.* **Same family as the `$847/mo` passive-plans figure and the eleven-day GA4 "July": arithmetically right, and it tells you something false.**
| Ahrefs Webmaster Tools: new warnings only | Ahrefs | No — glance, 5 minutes |
| ~~Members-area access counts (delta)~~ **Members-area: distinct active athletes (WAU/MAU), and page views by tool** | ~~`psql`~~ **`member_activity` / `member_tool_usage` on `analytics-postgres`** | **Yes** |

> ***Upgraded September 8, 2026. The old row named `access_count`, which cannot answer the question the row exists for*** — *it is a lifetime counter, so "8 have ever logged in" and "8 users in August" are the same number. `monthly-close/2026-08.md` §Known gaps recorded that limit and called `member_access_log` "parked"; **it was built September 5, 2026**, three days after the close was written. Close #1 stays as written — it was correct on its date — and this is the row that supersedes it.*
> **Windows, settled by Iván September 8, 2026 — calendar, not rolling.** ***MAU is the calendar month***, which is the only definition that composes with the rest of this close: every other row here is a calendar month, and a rolling 30-day window would make the members-area row the one number that cannot be compared to the P&L beside it. ***WAU is Monday–Sunday*** *and is* **not a close metric** — *it belongs to a future dashboard, where a weekly pulse is worth something. Do not put it in the close; a weekly figure reported monthly is four numbers pretending to be one.*
> **Availability, and it is a hard gate:** *the log starts* **2026-09-05**, *so* **September is a partial month (Sep 5–30, four days missing) and must be labelled as one.*** **The first complete calendar-month MAU is OCTOBER 2026**, reported at the October close in early November. ***There is no history before September 5 and none can be reconstructed*** — *`access_count` is a lifetime counter and cannot be decomposed into months, which is the whole reason the log exists.*
> ⚠️ ***And state the denominator problem rather than inheriting it.*** *At ~38 token holders, WAU/MAU is a metric designed for populations you cannot enumerate, and this one you can name.* **"13 of 38, and here is which tools each opened" is strictly more actionable than "34% MAU"**, *and a DAU/MAU stickiness ratio at this n is noise dressed as a KPI.* **Report the distinct count and the per-tool distribution; the ratio earns its place only if the book reaches a size where naming everyone stops being possible.** *The per-tool split is the row that changes decisions — it is the only evidence in this business about what an athlete actually wants from the library.*
| **Gemini API spend, and tokens by caller** | `model_usage` in the `content` DB *(view: `model_usage_daily`)* | **Partial — and know what the partial means.** *The tokens are exact and per-caller for the research, write and translate agents. **Hermes and the n8n intake briefing do not go through `call_model` and are absent from the table**, so a month that reconciles to the Studio total is evidence they are small, and a large gap is the measurement of how big they are. **Read `billed_output`, not `output_tokens`** — thinking is billed at the output rate and on a trivial probe ran 108:1 against output. Owner: `ai-infrastructure-documentation.md` §33 addendum* |
...
10. **Review velocity, not review count.** ~~45~~ **46** reviews is inventory. *(Count corrected Aug 14, 2026; owner `social-proof-and-reviews.md` §Live metrics — 46 total / 38 posted / 8 unused. The point this bullet makes is exactly why the copy went stale: nobody re-reads an inventory figure used only to argue that inventory is the wrong metric.)* *New reviews this month* and *asks made this month* is the process — and the July push produced 7 reviews in 2 days, so the process demonstrably works and demonstrably isn't running.
...
| 6 | **GA4 pull** | ~~Needs a query~~ **Built Aug 29, 2026** | The BigQuery export is already live and feeding the same GCP project as the plan-view pixel — `sync_ga4_data.py` now reads it into `analytics-postgres`. **This half needed no new credential, API or IAM grant at all**, because the export lands where `pixel-sync-vps` can already see it. *One thing to confirm once: that service account's BigQuery roles must be project-level, not scoped to `tracking_data`* |
| 7 | **GBP pull** | ~~Needs API setup~~ ~~**API approved Aug 17, 2026** — needs the script~~ **✅ DONE — live on cron since Aug 30, 2026** | Business Profile Performance API. *Application approved (ticket `8-0766000040742`); remaining work is Iván enabling the API in `training-plan-analytics` + granting the service account profile access, then a script in the shape of `automation/analytics/sync_pixel_data.py`. **Approval covers the API family, not specifically the local-post publishing endpoint** — that is a separate check, logged in `open-loops.md` NOW* |
| 8 | **GSC pull** | ~~Needs API setup~~ ~~**Built Aug 29, 2026 — needs Iván's enablement + verification**~~ **✅ DONE — live and verified Aug 30, 2026, history back to 2025-05-06** | The existing GSC↔GA4 link is **reporting-level only — no data flows**; that has not changed and is why the API was built. *Also settled that session: the GSC BigQuery bulk export is* ***forward-only***, *so it was rejected for this — it would have forfeited 16 months of history.* Steps and verification queries: `ai-infrastructure-documentation.md` §9 |
