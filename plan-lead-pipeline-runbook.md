...
~~**Status: NOT LIVE.**~~ **Status: LIVE · verified: 2026-08-14** (Iván — workflow Active in n8n, capture live on every plan page, `PLAN_CATALOG` present in Twenty's `leadSource` enum).

> ***Corrected August 14, 2026. This header read "NOT LIVE" for eight days after the backend went live on Aug 6.*** *`ai-infrastructure-documentation.md` §17's closing note recorded "email-capture backend live and tested end-to-end Aug 6" — and the same §17's body, this runbook's header, and `site/assets/js/plan-capture.js` all still said the opposite. **Three of the four copies were wrong and they outvoted the right one.*** *Steps 1–3 below are a completed build record, not a to-do list; read them as "how this was wired", not "what to do next". Live state now belongs to `open-loops.md`; this header carries a `verified:` date so the next reader can see its age.*

~~Front-end is built and shipped (every plan page has the capture form, `site/assets/js/plan-capture.js`, posting to `/api/plan-lead`). Nothing on the backend exists yet — this is the exact spec to wire it up, same pattern as `contact-form-pipeline-runbook.md`. Until Step 1-3 below are done, the capture form on every plan page fails silently into its "couldn't send" message.~~ **The "Buy on TrainingPeaks" button is a plain link with no dependency on any of this — it works today, live, regardless of this runbook.**
...
7. *(Corrected Aug 13, 2026, while wiring the zone-magnet clone: **the instruction below cannot work as written.** n8n registers the test webhook at `/webhook-test/<path>` and only registers `/webhook/<path>` once the workflow is Active — and Caddy rewrites to the production path. A real form submission while "Listen for test event" is armed 404s, which looks exactly like a broken Caddy route. Test by POSTing to `http://100.70.89.17:5678/webhook-test/plan-lead` over Tailscale instead, then activate, then submit from a live page as the end-to-end check. Full reasoning in `zone-magnet-runbook.md` Step 3.6.)* "Listen for test event" on the Webhook node, submit the form for real from a live plan page, confirm: a Person appears in Twenty with `leadSource = PLAN_CATALOG`, `excludeFromSequence = false`, `preferredLanguage = SPANISH`, and `leadNotes` carrying the plan name, plan id, **placement** and page URL; the reply email arrives **in the submitter's language** and asks the three qualifying questions (goal + date, days per week, recent training) rather than claiming to attach a plan; Telegram notifies you.
