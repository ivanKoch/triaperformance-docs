...
**Status:** LIVE **· verified: 2026-08-14** (Iván: research → his approval → writer → his approval → translator, all running). *Corrected Aug 13, 2026 — this line read "written, nothing deployed. July 27, 2026" for two and a half weeks after the fact. Both agents plus translation have been on cron since Aug 4, and the admin approval page (`/admin/ideas`, `/admin/drafts`) is running. Confirmed by Iván. **The stale line matters because it is the first line of the file:** anyone deciding whether to build on this engine read "nothing deployed" and stopped.*
...
~~You have six artifacts already gated (`carga`, `carrera`, `kettlebell`, `nutricion`, `tests`, `zonas`)~~ **You have eight entries gated in each of three languages** (`calculadora-de-zonas`, `carga`, `activacion`, `core`, `respiracion`, `rodillas`, `aquiles`, `guias` — plus their `en`/`pt` siblings), and All-Access has ~~2 subscribers~~ **1 subscriber (Portuguese)**. *(Corrected Aug 14, 2026: **five of the six names in the struck list were deleted on Aug 13, 2026** — only `carga` survives. This matters here specifically because the paragraph is telling the writer agent's operator which gated artifacts articles can point at, and `cta_target`'s datalist of members-tool paths is built from the real list. The subscriber figure came from Iván the same day — owner is `triaperformance-business-overview.md` §Revenue streams, and from the September close onward it is a monthly-close output rather than a hand-updated copy.)* This is the only content pattern that converts free search traffic into your one recurring revenue line.
