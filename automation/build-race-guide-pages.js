#!/usr/bin/env node
/**
 * build-race-guide-pages.js — renders the three members-area pages for the race
 * execution guide from the shared AST.
 *
 * Home doc:  race-execution-guide-brief.md
 * Content:   automation/race-guide-content.js (which reads the markdown)
 * Outputs:   site/members/ejecucion/index.njk           (es)
 *            site/members/en/race-execution/index.njk   (en)
 *            site/members/pt/execucao/index.njk         (pt)
 *
 * ⚠️ NEVER HAND-EDIT THE GENERATED PAGES. Edit the markdown in
 * race-execution-guide/ and re-run this. Every page carries a DO-NOT-EDIT
 * banner saying so.
 *
 * Usage: node automation/build-race-guide-pages.js
 */
...
const CHROME = {
  es: { dir: "site/members/ejecucion",         home: "/members/#biblioteca",    lib: "Biblioteca", crumb: "Ejecución de carrera", label: "Guía",  toc: "En esta guía", pdf: "Descargar en PDF" },
  en: { dir: "site/members/en/race-execution", home: "/members/en/#biblioteca", lib: "Library",    crumb: "Race execution",       label: "Guide", toc: "In this guide", pdf: "Download as PDF" },
  pt: { dir: "site/members/pt/execucao",       home: "/members/pt/#biblioteca", lib: "Biblioteca", crumb: "Execução de prova",    label: "Guia",  toc: "Neste guia",   pdf: "Baixar em PDF" },
};

/* members-combustible.css is the long-guide page stylesheet, not a fuelling-
   specific one — it styles exactly the markup these generators emit. Reused
   rather than duplicated. ⚠️ Its NAME is now wrong for what it does; renaming
   it touches a live page, so it is logged in race-execution-guide-brief.md §13
   rather than done here. */
const GUIDE_CSS = "members-combustible.css";
...
function buildPage(lang) {
  ...
  const body = rest.slice(0, -1);             // the 16 numbered sections
...
{#- GENERATED FILE — DO NOT EDIT BY HAND.
    Source:    race-execution-guide/${c.source}
    Generator: automation/build-race-guide-pages.js
    Home doc:  race-execution-guide-brief.md
    ...
    The scope box on this page is a copy of methodology.md §14; the pacing,
    taper and debrief rules are copies of §9. Move them there first, then in
    the markdown. -#}
...
for (const lang of Object.keys(CHROME)) {
  ...
  // The stop list is why methodology.md §11 got a dated exception. A page that
  // lost it would be a guide telling athletes to push on with no counterweight.
  if (!/callout--warn/.test(html.split("</header>")[1] || "")) {
    throw new Error(`${lang}: no warn callout in the body — the stop list did not render`);
  }
