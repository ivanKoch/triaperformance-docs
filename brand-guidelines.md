...
- **Carbon on public pages is allowed on exactly four surfaces**: the **hero**, the **final CTA**, the **featured offer card**, and the **public copy of a members tool**. A page may use the hero and the final CTA together — they bookend it. Everything between them stays white-first.
- **The fourth surface carries a test, because it is the one that can be argued into anything.** An *app-shaped* tool — a timer or a routine held in one hand, mid-session, on a floor — keeps carbon, because a light clone of a carbon members tool is a different product wearing its name. A *document-shaped* tool — a form and a table you read, like the zones calculator and the pace converter — follows the page and stays light. **The question is not what the page contains, it is what the athlete is doing while they look at it.**
