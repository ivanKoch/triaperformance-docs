...
| **Duplicating an article the site already has** | ***Happened Aug 18, 2026.*** The research agent has no knowledge of published articles — `research_agent.py` loads plans, members artifacts, races and methodology, and never the blog. It proposed a near-clone of `/blog/entrenar-el-umbral-sin-tiras-de-lactato/`, which passed Gate A because **Gate A cannot reject a duplicate it has no way to recognise.** Fix (open): feed the published list into OUR ASSETS. |
| **A draft contradicting `methodology.md`** | ***Happened Aug 18, 2026, same article.*** It prescribed a 20-minute running threshold test with a 0.95 multiplier — the cycling FTP convention — against a documented 30' protocol, and it reached the live site. Gate B is a human read and it passed. Fix (open): a deterministic check of any stated duration or multiplier against the §Test protocols table, run before Gate B rather than relying on it. |
