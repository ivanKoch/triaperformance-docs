...
2. **New finding, not in the original dead-plan lists above:** plan_ids 612974 (Lima Marathon, Spanish) and 612836 (Lima Marathon, Portuguese — see the dedup note below) both now return "Training Plan Not Found" despite `is_published = TRUE` in the inventory. Same pattern as the resolved 434680/443810-family issue above — needs the same manual TP-side check, not yet done.

**Dedup status correction.** This section previously said the 4 known duplicate `plan_id`s (439394, 439396, 439397, 612974) were "left as-is; no dedup logic applied yet" in `plans_raw`. That was accurate for Postgres but became stale for the *source CSV*: `plan-storefront-project-brief.md` §3 and git commit `a3833e8` (July 22, 2026) show the CSV was already fixed — 439394/439396/439397 each had a stale duplicate row with `link = "Expired"` removed (the real-link row for each was kept), and 612974's second row (a copy-paste error — a Portuguese plan wrongly carrying the Spanish plan's ID) was corrected to its real ID, 612836. Current source CSV (381 rows) has **zero duplicate `plan_id`s**, confirmed directly (`sort | uniq -d` on the `plan_id` column). `plans_raw` in Postgres was loaded July 21, one day *before* that CSV fix, so it's the only place this still needs to be applied — diagnostic + fix SQL prepared, pending Iván running it and confirming row counts (see handoff / open-loops).
