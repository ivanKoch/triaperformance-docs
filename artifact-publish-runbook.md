...
| Recovery day, active (activation engine) | `/members/recuperacion/` · `/members/en/recovery/` · `/members/pt/recuperacao/` | activacion | ES Sept 4, **EN+PT Sept 5, 2026**. 15 routines, 5 sports × 30/45/60 min, three named blocks. Section B of the same source doc as `/members/movilidad/`. EN/PT derived by `automation/recovery-i18n.py` (names and cues by position, verified against the Spanish). Home doc: `recovery-brief.md` |
| Pace converter | `/members/conversor-de-ritmo/` · `/members/en/pace-converter/` · `/members/pt/conversor-de-ritmo/` | — (own component) | Sept 8, 2026, all three, **plus a public version at `/conversor-de-ritmo/` + `/en/pace-converter/` + `/pt/conversor-de-ritmo/`**. One partial, two skins — `pcDark` on the members copies, same arrangement as the zones calculator. Closing note and the five decisions: `open-loops-archive.md`, Sept 8 |
| Garmin setup (guide) | `/members/garmin/` · `/members/en/garmin/` · `/members/pt/garmin/` | — (prose) | *Row added Sept 8, 2026. Live in all three languages and sold as live in `library.json`; it had never been in this table or in `triaperformance-business-overview.md` §Interactive tools — the fourth consecutive miss by the pair. See the note below.* |
...
> 🚨 ***Third correction, September 8, 2026 — and it is the second one's own prediction coming true.*** *That note ends by saying the check which would have caught the August miss is now Step 0 of `kb-hygiene-prompt.md`.* **Step 0 diffs `library.json` against `site/members/`. It does not diff either inventory table, which is the surface that has now failed four times** — `cyclistcore`, `strength`, `hombro`, and today `garmin`, which has been live in three languages for weeks. *Nothing was mis-sold this time, and nothing would have caught it if it had been.* ***The fix is not a fourth note: it is to render these two tables from `library.json` at build time or delete them and name that file the inventory of record.*** *Logged as its own item in `open-loops.md`, September 8, 2026.*
