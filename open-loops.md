...
- [ ] **Two of the six downloadable guides are Spanish-only — the mental-training pair** (`guia-de-herramientas-mentales.pdf`, `intervalos.pdf`). The fuelling guide showed the cheaper route: rebuild on the markdown→PDF generator so all three languages ship at once, rather than translating a hand-built PDF.
...
- [ ] **No backups on the VPS.** Twenty, `analytics-postgres` and n8n hold every customer record, the warehouse and every workflow. Recorded at $0 in close #1 as a risk. Iván: after this cleanup.
