...
**Last updated: July 29, 2026 (maintenance session — token dedup, All-Access dates, CoachMatch location/BR-AR fixes).** This file replaces the open-item lists previously scattered across `ai-infrastructure-documentation.md`, `growth-roadmap.md` (Sequencing), `content-engine-brief.md` (Open decisions), and the runbooks. Those docs keep the *detail*; this file is the *list*. Update it at the end of every working session.
...
- [ ] Eleventy Phase 5 cleanup — **in progress July 29, 2026.** Scope changed from the original plan: `hubfs` is now permanent (TrainingPeaks marketplace hotlinks it across ~300 live plans — confirmed with Iván, do not move it, ever). Only `images` and `guias` move into `site/assets/`. Code side done: `.eleventy.js` passthrough updated, all internal `/images/`/`/guias/` references repointed to `/assets/...`, and a new gated landing page built at `site/members/guias/index.njk` (Spanish only, listing the 3 actual PDFs, correctly inheriting the members dark nav/`noindex` — confirmed with Iván the listing belongs behind the members gate, not as a public `/guias/` page) since the old bare `/guias/` link had no index page and always 404'd. Still open: Iván deletes the stray `site/guias/` folder (created then corrected mid-session, never committed — plain `rm -rf site/guias`, not `git rm`), runs the `git mv` for `images`/`guias`, confirms local build including `/members/guias/`, applies the Caddy redirect for the 3 known old PDF URLs (`/guias/<file>.pdf` → `/assets/guias/<file>.pdf`, file-specific), then deletes the superseded `website/planes`, `website/pt`, `website/all-access`, `website/members`, `website/en` folders (not all of `website/` — `hubfs` stays). Detail: `website-build-cutover-runbook.md` steps 17–19.
- [ ] **New, found July 29, 2026:** the 3 downloadable guides only exist in Spanish, but the EN and PT blog articles link to the Spanish PDF anyway (pre-existing gap, not caused by anything this session touched). Needs either real EN/PT translations of the guide or a decision to just not link it from those articles. Also noticed in passing: the members-area copy describes the guides as "Nutrición, Prep. Mental y Zonas" but the actual 3 files are zonas-de-entrenamiento/pre-entreno/intervalos — copy doesn't match content, worth a quick look whenever this gets picked up.
