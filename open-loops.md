...
- [ ] **Iván: read the week grid on page 4 of the runner's strength week guide.** Derived from his placement rules in `methodology.md` §13.1 and never signed off as a week. Five minutes. Home doc: `lead-magnet-semana-de-fuerza.md`.
...
- [ ] **Read the EN and PT members FAQ** (`/members/en/#faq`, `/members/pt/#faq`). Customer-facing copy written September 4, sent to Nadine on September 7 unread. Descriptive, not new policy. Ten minutes, or downgrade to Backlog.
...
- [ ] **The race execution magnet has no public page to sit on.** The guide is gated; `/api/tool-lead` plumbing is two edits (`site/_data/capture.json` plus the n8n Magnet registry) but a capture form needs a public host page. Candidates: a new article, a 42 km race page, or `/planes/`. Blocked on Iván. Home doc: `race-execution-guide-brief.md`.
- [ ] **The library has no pacing tool.** `/members/carrera/` was retired August 13 and nothing replaced race pacing / the race plan by distance. Fuelling half closed September 11 (`/members/combustible/`, `fueling-guide-brief.md`). Do not restore the old page as-was.
...
- [ ] **Measure the CoachMatch PT backlog send (30 emails, September 6).** Follow-up to non-responders around September 12; read subscriptions, `utm_campaign=pt-backlog-2026-09` sessions and unsubscribes at the September close. Do not set these leads back to `MESSAGE_SENT` — that enrols them in the nurture.
- [ ] **Lead-magnet emails carry no click attribution.** Use the existing `/c/<code>` redirector (`automation/Caddyfile`, logs to `campaign_link_clicks`), not UTMs — it also works on the PDF link, which GA4 never sees. Two codes cover the magnet emails. Rides along: the three Telegram nodes parse Markdown and eat underscores (`runner_week` → `runnerweek`); set parse mode to HTML or none.
...
- [ ] **Apply `automation/members-area/schema.sql` to the live `members` DB.** Check first: `docker exec -i analytics-postgres psql -U analytics -d members -c '\dv member_home_visits'` — if the view exists, close this. If not: `cd ~/.hermes/triaperformance-docs && git pull && docker exec -i analytics-postgres psql -U analytics -d members < automation/members-area/schema.sql`. Idempotent. Must be live before the September close.
- [ ] **`ivankoch87@gmail.com` is not flagged `excluded_from_metrics`, so Iván counts as the most active member on the site.** The revenue side already subtracts him (roster → 37); the usage side includes him. Decide, then make both sides agree; check the Bogotá tester in the same query.
- [ ] **`members_access_gap.py --expect N` is exact-equality and goes stale weekly as Twenty grows.** Either drop it and rely on the `totalCount` guard, or change it to `--expect-min`. Ten minutes; next hygiene pass at the latest.
...
- [ ] **The weekly hygiene pass's scheduled task has no folder bound** (`trig_01Pfo8TnD7Hoj6wCnXEp22VM`, `folders: []`), so it fails at step zero every Tuesday. Folders attach at approval only: recreate it from a desktop session with the repo connected, delete the old one. Iván's.
- [ ] **Two stale test records at `MESSAGE_SENT` in Twenty** (`coach+zone-en@`, `coach+zone-pt@`, `ZONE_CALCULATOR`). No daily noise (no phone), but they inflate any funnel count. Delete or move to a terminal status.
...
- [ ] **Build athlete-facing FAQs and help articles reactively** (Iván, Sept 3). No `/faq/` or `/ayuda/` exists in any language. Rule: nothing goes on the page until a real athlete has asked it; the Monday check-in is the collection point. Pre-sale objections stay in `sales-playbook.md` — different audience.
...
- [ ] **No backups on the VPS — script and runbook written September 11, 2026, not yet run.** `automation/backup-vps.sh` + `deploy-runbook.md` §5. Iván: the setup block (passphrase into Bitwarden, rclone → Google Drive), one manual run, the restore test, then the crontab line. Closes on a verified restore.
...
- [ ] **`quick_grant.py` exists only on the VPS at `~/.hermes/`, with no repo copy.** Move it into `automation/members-area/` or delete it — `OPERATIONS.md` §1 already documents three granting paths. Read it first: if it prints a token to stdout it is a leak surface.
