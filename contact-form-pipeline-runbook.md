...
- **Sport dropdown mapping (added Jul 22, 2026):** the form's `Deporte` field now has exactly 4 options, shown in Spanish to the visitor: `Running`, `Ciclismo`, `Duatlón`, `Triatlón`. Twenty's `sportPrimary` field uses English values: `Running`, `Cycling`, `Duathlon`, `Triathlon`. The **Build Twenty payload** node now translates one to the other before writing to Twenty (confirmation email and Telegram notification still show the original Spanish word to keep those human-readable). If `sportPrimary` in Twenty is a Select field, confirm its option values are exactly `Running` / `Cycling` / `Duathlon` / `Triathlon` (case-sensitive) — a mismatch will fail the Create Person call with a validation error, caught immediately by the Step 2.6 test.
