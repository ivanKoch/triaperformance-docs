...
🚨 ***SECOND INSTANCE, September 11, 2026 — and it is the more instructive one.*** *The September 9 correction above fixed the footer. It did not fix `automation/unsubscribe-workflow.json`, which carried voseo on **both pages this system serves**: the success page (`Build Page` node — "si **querés** volver a recibirlos, **escribime**") and the invalid-token page (`Respond Invalid Link` — "ya expiró. **Escribime** a"). Both were live and customer-facing for five days, and were found only because Iván read his own unsubscribe confirmation while testing an unrelated lead magnet.* ⚠️ **`register-sweep.py` had the mappings for every one of those forms and flags this file at two lines. It had simply never been run over it.** 🔑 ***The correction was applied to the string someone was looking at, not to the file it lives in*** *— which is the same shape as every checker lesson in this repo, one level up: the fix, like the check, only reaches the path it sits on.* **When a register defect is found in a surface, sweep the whole file, not the line.**
