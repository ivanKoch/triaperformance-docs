...
#### Fourth pass: a crawl of a deploy that predated the work, and two defects inside it

**Iván: "my request to create a better way to display plan alternatives still hold."** ⚠️ ***It was already built and committed*** *(`6f9ae43`), and the crawl behind the note is of a deploy that predates it — its own "still true" list names the `¿No te encaja del todo?` text links, which the `También puedes` row replaced.* 🔑 **The same note's "fixed" column describes the rail, the week object, the All-Access line and the heat long-run figure as live, so it straddles two deploys.** *That is worth naming once as a working rule:* **an audit of the deployed site is not an audit of the repo, and on a daily-cron deploy the two are a day apart.**

**Two real defects came out of it, neither the one it named.**

🚨 **71 of 328 plan pages printed the same number twice in the rail.** *41 ES, 22 EN and 8 PT plans are priced at exactly the All-Access figure, so the box read* `US$ 39.99 pago único` *above* `US$ 39.99/mes` *— two identical numbers a few lines apart, which reads as a bug rather than an offer.* **The number now leaves the line on those pages only** (`¿Más de un objetivo? Mira All-Access`), *and stays everywhere else.* ⚠️ ***The tempting fix was refused:*** *"por el mismo precio, todos los planes" would have turned the collision into the strongest line on the page, and it is a false equivalence — $39.99 once and $39.99 a month are the same number, not the same price, and this is the page whose only job is a purchase.* **Measured after: 0 pages.**

🚨 **The utility strip was still `--wash` — a pale blue band directly above a carbon nav — on the three PUBLIC core pages.** *The carbon version of that block already existed, in `members-dark.css`.* 🔑 ***The two files are the site's two carbon shells and they serve disjoint page sets:*** *16 pages load `members-dark.css`, the tool pages load `members-activacion.css` through `members-fuerza.css`'s `@import`, and* **no page loads both** *— so the nav was themed in one file, the strip above it in the other, and each page got whichever half its stylesheet carried.* **Same defect family as September 9's nav rule, one level up: there a rule did not reach a sibling element, here it did not reach a sibling page.** *Fixed as a deliberate second copy with a cross-reference in both files; the structural fix is one body-level carbon class in `site.css`, which needs a hook `base.njk` does not have.*

**And the claim that was checked and did not hold.** *The note said the carbon nav tokens were "not visible on this deploy" and that `Recursos` is "still dim grey".* **Sampled from rendered pixels: nav links and the `Recursos` dropdown label are `--text-dim` on `--bg` at 5.85:1, which clears §3.4's 4.5:1 gate.** *They ARE dim — that is what a secondary token is for — but dim and failing are different properties.* ⚠️ ***My own first measurement said 3.8:1 and was wrong:*** *the nav is `rgba(--bg,.85)`, and a computed-style walk composited that translucency over white.* **Third time today that a computed-style contrast reading has been wrong and a pixel sample has been right.** *Stop computing contrast on anything translucent.*

**Still open from that note, and deliberately not done:** the TP banner PNGs *(decided against on the files themselves, twice)*; a sticky rail *(tried and measured away)*; a "next distance" card slot *(needs a per-sport distance ladder that does not exist in the data — a real addition, Iván's call)*; the generic 80/20 block on every plan *(coaching copy `methodology.md` owns)*; and one help CTA instead of WhatsApp + form *(a fair point, and a decision about how a hesitant buyer reaches him rather than a layout bug)*.
