...
- ~~Upload plan_performance.csv → table on Hostinger VPS~~ **Done Jul 21, 2026** — `storefront` database on the existing `analytics-postgres` container, `plans_raw` (386 rows, cleaned) + new `plan_weekly_breakdown` table (TP's per-plan workout stats, never in the CSV before — 190/330 plans crawled so far). Full detail: `ai-infrastructure-documentation.md` §10, `plan-storefront-project-brief.md`.
- ~~One dynamic plan-view template renders all plans~~ **Built July 30, 2026, not yet deployed.** 321 published/linkable plans (179 ES / 108 EN / 34 PT), one shared template paginated per language at `/planes/p/<slug>/`, `/en/plans/p/<slug>/`, `/pt/planos/p/<slug>/`. Detail: `ai-infrastructure-documentation.md` §17.
- ~~Facet filters: sport, distance, difficulty, weeks, language, features~~ **Built July 30, 2026.** Client-side, static (no backend) — sport/distance/difficulty/weeks/features per language catalog (language itself isn't a facet since each catalog page is already single-language). The 4 existing hand-written category pages converted to the same component, same URLs preserved.
- ~~Email capture (lead magnet / "email me this plan") BEFORE the TP redirect; all redirects carry UTM + plan_id~~ **Front end built July 30, 2026; backend not live.** Inline, skippable capture next to the buy CTA (never blocks the redirect). UTM + plan_id confirmed on every plan's TP link. Backend (n8n workflow, Caddy route, Twenty `leadSource` enum addition) spec'd in `plan-lead-pipeline-runbook.md`, not yet wired up.
- ~~All-Access promo module on every plan page~~ **Built July 30, 2026.** Condensed module on every plan page linking to the existing per-language All-Access landing pages (dedicated landing pages already existed, pre-storefront).
- SEO: hub pages by intent per language (ES first), hreflang, Product schema with prices + review stars from the social-proof quote bank. **Partially done July 30, 2026:** Product schema (price + availability, no `aggregateRating` yet — review-per-plan sourcing still unresolved, see `plan-storefront-project-brief.md` §5 open questions) ships on every plan page; Rich Results Test validation needs a live URL, next session. Intent hub pages not started.
