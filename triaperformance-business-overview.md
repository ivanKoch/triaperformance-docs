...
- **Interactive tools** — the members library. **Inventory of record: `site/_data/library.json`** (one entry per tool, three languages, `gated`/`live` flags); the three All-Access sales pages and the members homes render from it, so it is right by construction. Two engines in the repo: `activation-tool.js` (timed circuits) and `strength-tool.js` (sets × reps + rest timer). Each tool has a home doc named in `triaperformance-project-instructions.md`; process in `artifact-publish-runbook.md`. *The hand-maintained table that used to sit here missed four tools in a row and was removed September 11, 2026 — do not rebuild it; read the JSON.*
- **47 GBP reviews, Google average 5,0** (46 five-star + one four-star from September 2026; zero negatives; owner `social-proof-and-reviews.md`; **cosmetic figure, not reconciled**) — 38 repurposed to Instagram, 8 never posted. Curated quote bank, gaps (3 English, 0 Portuguese), and deployment plan live in `social-proof-and-reviews.md`. The July 2026 review push produced 7 reviews in 2 days — asking works. Supply is not a constraint: ~22 active athletes have never been asked, ~14 are due an ask in the next cycle, and already-posted reviews are freely recyclable after ~a year.
