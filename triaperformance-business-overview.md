...
- **Interactive tools** — ***the members library was rebuilt on August 13, 2026 and now stands at*** ~~nine entries in each of three languages: seven interactive tools, one guide, one downloads page~~ ~~ten entries in each of three languages: seven interactive tools, two guides, one downloads page~~ ***sixteen entries in each of three languages — 2 calculators, 9 routines, 1 tool, 3 guides, 1 downloads page (September 8, 2026).*** *Breakdown re-derived from `site/_data/library.json`'s `tag` field rather than counted by hand, because that is the file the three sales pages actually render from: if this line and that file disagree, the visitor sees the file.* ~~plus a fourteenth, the runner core routine, in Spanish only (September 5, 2026)~~ — **runner core has been in all three languages since September 4 and the row below already said so; the count line had not caught up.**
 ...
 > 🚨 ***FOURTH INSTANCE, September 8, 2026, found the same way as the second and the third — by opening this table to add an unrelated row.*** *`/members/garmin/` — the Garmin setup guide — is live in all three languages, is sold as live and gated in `library.json`, and has never appeared in this table.* **It is not a new page: `site/members/garmin/`, `site/members/en/garmin/` and `site/members/pt/garmin/` all exist and all three languages are wired.** *So the table has now missed four entries in a row, and the count line above it was separately wrong about a fifth (runner core).* ⚠️ ***The three earlier notes each diagnosed the pattern correctly and none of them changed it, which says the problem is not that people forget — it is that this table is hand-maintained prose in a document nothing reads at build time.*** **`library.json` is machine-read by the build and is right every time; this table is read by humans and has been wrong four times.** *The fix is not a fifth note: it is to generate this table from `library.json` at build time, or to delete it and point at that file. Logged in `open-loops.md`.* **Garmin's row is added below, and the count above is re-derived from `library.json` rather than incremented.**
  ...
  *(Rewritten that day. This paragraph previously read "nine are live" and listed as unbuilt several things that now exist.)*
  ...
  | Pace converter | `/members/conversor-de-ritmo/` | `/members/en/pace-converter/` | `/members/pt/conversor-de-ritmo/` | min/km ↔ min/mile ↔ km/h ↔ mph, live in four synced fields, plus the two nearest treadmill settings and the pace each really gives. **All three languages September 8, 2026.** Public version at `/conversor-de-ritmo/` — this is the dark copy of the same component |
  | Garmin setup | `/members/garmin/` | `/members/en/garmin/` | `/members/pt/garmin/` | Your seven zones into the watch's five fields, by sport, and how TrainingPeaks and Garmin coexist. *Row added Sept 8, 2026 — see the fourth-instance note above* |
