...
- **Website**: migrated off HubSpot, live on the Hostinger VPS (same box as Hermes) behind Caddy at `triaperformance.com`, auto-deployed via a daily cron pull+sync (see `deploy-runbook.md`). *Updated Aug 2, 2026:* the site is now an Eleventy build sourced from `site/` (not the old hand-written `website/` folder), with templated analytics, canonical/hreflang and sitemap. Live: homepage, three-language blog, per-language All-Access landing pages, category pages, a contact form wired into Twenty, and a token-gated `/members/` area with the interactive tools. Built but not yet deployed: the dynamic plan catalog + individual plan-page template (July 30). The largest open item is the Plan Storefront, not the page set — see `open-loops.md`.
