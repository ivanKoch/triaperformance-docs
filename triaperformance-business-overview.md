...
- **Interactive tools** — ***the members library was rebuilt on August 13, 2026 and now stands at*** ~~nine entries in each of three languages: seven interactive tools, one guide, one downloads page~~ ***~~ten entries in each of three languages: seven interactive tools, two guides, one downloads page~~ twelve entries: nine interactive tools, two guides, one downloads page — and eleven of the twelve exist in three languages, with `/members/movilidad/` Spanish-only by design (September 3, 2026).***
  ...
  *(Rewritten that day. This paragraph previously read "nine are live" and listed as unbuilt several things that now exist.)*
  ...
  | Mobility (post-exercise) | `/members/movilidad/` | — | — | 2 questions → 15 routines, 10/20/30 min. **Spanish only; EN/PT deliberately not claimed anywhere.** Home doc: `mobility-brief.md` |
