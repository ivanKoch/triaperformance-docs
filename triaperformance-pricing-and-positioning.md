...
Because the service is uniform rather than tier-differentiated, onboarding a new coach doesn't require inventing a restricted scope — the standard service described above is already well-bounded and is what a hired coach would deliver. The economics only work at the $149 price point, though: at 3.5% commission (Private channel, via TP Payments), $149 nets $143.79, split 50/50 is $71.89 to the coach and $71.89 to Iván. At $89–109, the split gets thin fast and likely isn't worth either side's time. *(~~A **referred** Private athlete~~ **EVERY Private athlete, from September 4, 2026,** carries a further $9 for TrainingPeaks Premium — $134.79, i.e. **$67.40/side**. *The referred/not-referred split this line used to draw no longer exists.** ***Where that cost falls is a live question:*** *taking the split after Premium is symmetric at $67.40 each; taking it before hands the coach $71.89 and leaves Iván $62.90 for the same work. Not decided.)* 
...
| Private | $149 | 3.5% (TP Payments) **+ $9 Premium** | **$134.79** | **$67.40 / $67.40** — viable |

*~~A separate `Private, referred` row carried $134.79 here between Aug 26 and Sept 4, 2026.~~ **Deleted, not struck: the distinction it described no longer exists** — Premium is now on every Private athlete, so the Private row above IS that row.*

The same athlete is worth **$15.59/month more on Private** ~~$24.59~~. ⚠️ ***That gap narrowed by 37% on September 4, 2026 and the consequence is worth stating where the number lives:*** *every argument in this repo for routing an athlete to Private rather than CoachMatch — the referral blurb's checkout link, the website funnel — was sized against $24.59. **It is still the right call and it is a smaller prize than the documents saying so were written to describe.*** *The strategic case is unchanged; the arithmetic behind it moved.* Private is therefore still the priority acquisition channel; the website, lead magnets, and referrals all feed it. CoachMatch stays as-is ("pick Bronze, I'll give you Gold service level" against TP's public $149/$229/$359 ladder — $149 reads as the cheap option there).

## TrainingPeaks Premium — included for every 1:1 athlete, from September 4, 2026

🔑 ***Every 1:1 athlete has TrainingPeaks Premium, whether they arrived through CoachMatch or Private. That single sentence is why the claim is safe on every surface*** *— the homepage, both coaching cards, `sales-playbook.md` A5's CoachMatch and Private blocks alike.*

**WHO PAYS IS THREE DIFFERENT ANSWERS, and confusing them is what has gone wrong here twice.** *(Stated by Iván, September 5, 2026, to end it.)*

| Channel | Who pays for Premium | What it costs Iván | Since |
|---|---|---|---|
| **CoachMatch** | **TrainingPeaks**, out of the 20% it takes on whatever the athlete pays ($75 / $100 / $149 — the rate does not matter) | **$0** | always — this is not new |
| **All-Access** | **TrainingPeaks, compulsorily.** It deducts **3.5% + $9** before settling to Mercury | **$0 incremental** — it is already inside the net | always |
| **Private 1:1** | **Iván**, $9/month out of pocket | **$9 per athlete** | **September 4, 2026** — this is the only thing that changed |

⚠️ ***The Private line is the only one that scales with headcount.*** *Read it at each close as Private grows; nothing else here moves.*

*Motivation, in Iván's words: the referral case made it obvious. A referred athlete was getting Premium and an identical $149 athlete arriving another way was not — a service difference keyed to acquisition channel, which is exactly what pricing rule 4 forbids.*

---

**Historical note — the Aug 26 to Sept 4 scope, kept because its reasoning is what produced the current answer.** *For nine days Iván bought Premium **for referred athletes only**. The reason was not parity: TrainingPeaks pays for Premium on CoachMatch athletes, most of the book is CoachMatch, so the athlete doing the referring almost always had Premium and described the service to a friend as including it — **the $9 existed to make the referrer's description true.*** **That was a good reason, and following it to its conclusion is what ended it:** *if the referrer's description has to be true for their friend, it has to be true for everyone, or the service differs by how you arrived.* *(An even earlier version applied the $9 to all Private athletes, was corrected to referred-only the same day, and is now back where it started by a different route — the reverts are in this file's history.)*

**Why the old scope was what it was, and it was not a parity argument.** TrainingPeaks pays for Premium on **CoachMatch** athletes and not on Private ones. **Most of the current book is CoachMatch** — so the athlete doing the referring almost always has Premium, and describes the service to their friend as including it. **The $9 exists to make the referrer's description true.** *That is a narrower and better reason than "the same service should be the same everywhere," and it is Iván's.*
...
✅ **Where it appears in copy NOW, complete as of September 5, 2026:** *`/referidos/` (unchanged — its list is headed "El mismo plan"), all three homepages on both coaching surfaces, and* **`sales-playbook.md` A5 in BOTH blocks** — *Private since Sept 4, CoachMatch since Sept 5.* ⚠️ ***The CoachMatch block was the one that mattered and it was the one nobody checked:*** *an open item read "A5 does not mention Premium", a session opened A5, found the line in the Private block and would have closed it — while the message half the leads actually receive still said nothing about a $19.95 inclusion TrainingPeaks was already paying for.* **Name the block, not the section.**
...
5. **Legacy rates are grandfathered — until a pause.** Existing athletes at $74.50/$89/$99 keep their rate for as long as they stay continuously active. An athlete who pauses returns at $149. **No exceptions** — this rule is the migration mechanism and only works if it's absolute.
   ...
   ✅ ***First real test of rule 5, September 9, 2026 — it held, and the athlete converted without a single question.*** *A 2024-vintage private athlete who paused in July 2026 returned for a new goal and was re-quoted at $149 against a legacy rate of ~$89–99. **What made it work was not the rule, it was the arithmetic being smaller than it looked:** he had been paying TrainingPeaks Premium himself ($19.95/mo standalone), so his real out-of-pocket went from ~$119 to $149 — **+25%, not the +50% the two coaching prices imply.*** ⚠️ **That reframing only exists for athletes who joined before September 4, 2026**, when Premium became an inclusion; every athlete quoted after that date already has it, and a future re-entry conversation will face the raw increase with nothing to offset it. *Concessions used: the $50 setup fee, waived and **named** (a silent waiver buys nothing). Concessions refused: any intermediate number — $119/$129 burns the rule and leaves the money.* **The old rate was never mentioned in the message, deliberately — see `sales-playbook.md` §B9's standing point that anything prompting an athlete to calculate their rate is a risk to the one policy that quietly converts the book.*
